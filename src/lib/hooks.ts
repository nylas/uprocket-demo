...
export function useUpdateUser() {
  const { data, error, trigger, isMutating } = useSWRMutation('/api/me', updateUser, {})
  return {
    user: data,
    isLoading: !error && !data,
    isMutating,
...
export function useSchedulingConfig() {
  const { data, error, mutate, isValidating } = useSWR<Partial<SchedulingConfig>>(
    '/api/config',
    fetcher,
  )
  return {
...
export const usePreBookedEventDetails = () => {
  const { data, mutate } = useSWR('preBookedEventDetails', { fallbackData: null })
...
export const useConfirmedBookingDetails = () => {
  const { data, mutate } = useSWR('confirmedBookingDetails', { fallbackData: null })
